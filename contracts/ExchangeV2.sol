    ...
    GhostAuction
{
    ...
    function __ExchangeV2_init(
        ...
        __GhostMarketTransferManager_init_unchained(
            newProtocolFee,
    ...
    function endAuctionDoTransfer(
        ...
        uint256 auctionId,
        bool notLazyMint
    ) external payable nonReentrant whenNotPaused auctionComplete(auctionId) {
        ...
        if (notLazyMint) {
            ...
            matchAndTransfer(
                ...
                orderRight,
                amount,
                auctions[auctionId].auctionSpecAddr[0]
            );
        } else matchAndTransfer(orderLeft, orderRight, amount, address(this));
    ...
    function createBid(
        ...
        uint256 amount,
        bool useToken
    ) external payable nonReentrant whenNotPaused auctionExists(auctionId) {
        ...
        require(auctions[auctionId].startingAt <= block.timestamp, "ANS");
        ...
        if (
            auctions[auctionId].auctionType ==
            LibExchangeAuction.AuctionTypes.Reserve ||
            auctions[auctionId].auctionType ==
            LibExchangeAuction.AuctionTypes.Classic
        ) {
            ...
            if (auctions[auctionId].amount == 0) {
                ...
                if (useToken) {
                    token.transfer(
                        address(auctions[auctionId].bidder),
                        auctions[auctionId].amount
                    );
                } else {
                    address(auctions[auctionId].bidder).transferEth(
                        auctions[auctionId].amount
                    );
                }
            }
        ...
        if (
            auctions[auctionId].auctionType ==
            LibExchangeAuction.AuctionTypes.Dutch
        ) {
            /// only one bid allowed
            require(auctions[auctionId].firstBidTime == 0, "DACOHOB");
            uint256 price = LibExchangeAuction.getCurrentPrice(
                auctions[auctionId]
            );
        ...
        if (
            auctions[auctionId].extensionPeriod != 0 &&
            auctions[auctionId].auctionType !=
            LibExchangeAuction.AuctionTypes.Dutch
        ) {
            if (
                auctionEnds(auctionId) <
                block.timestamp.add(auctions[auctionId].extensionPeriod)
            ) {

...
describe('ERC721WithRoyalties', () => {
  ...
  describe('Royalties', async () => {
    it('has all the right interfaces', async function () {
      expect(await erc721WithRoyalties.supportsInterface(_INTERFACE_ID_ERC165), 'Error Royalties 165').to.be.true;

      expect(await erc721WithRoyalties.supportsInterface(_INTERFACE_ID_ROYALTIES_EIP2981), 'Error Royalties 2981').to.be
        .true;

      expect(await erc721WithRoyalties.supportsInterface(_INTERFACE_ID_ERC721), 'Error Royalties 721').to.be.true;
    });

...
describe('GhostMarketTransferManager', async function () {
  ...
  beforeEach(async function () {
    ...
    if (hre.network.name == 'testnet_nodeploy' && do_not_deploy) {
      ...
      await testing.__TransferManager_init(
  ...
  it('From ETH(DataV1) to ERC721(RoyalytiV1, DataV1) Protocol, Origin fees, Royalties', async () => {
    ...
    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      ...
      let tx = await matchSigner.checkDoTransfers(
        left.makeAsset.assetType,
        left.takeAsset.assetType,
        [200, 1],
        left,
        right,
      ...
      await verifyBalanceChange(accounts2, 300, async () =>
  ...
  it('should buy ERC1155 with ETH; protocol fee and royalties', async () => {
    ...
    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      console.log('test1');
      let result = await matchSigner.checkDoTransfers(
        left.makeAsset.assetType,
        left.takeAsset.assetType,
        [200, 4],
        left,

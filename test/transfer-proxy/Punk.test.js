...
describe('Exchange with PunkTransfer proxies', () => {
  ...
  it('Check punk event', async () => {
    await cryptoPunksMarket.connect(signers[accounts[1]]).getPunk(punkIndex); //accounts[1] - owner punk with punkIndex
    let resOffer = await cryptoPunksMarket
      .connect(signers[accounts[1]])
      .offerPunkForSaleToAddress(punkIndex, 5, proxy.address); //accounts[1] - wants to sell punk with punkIndex, min price 0 wei

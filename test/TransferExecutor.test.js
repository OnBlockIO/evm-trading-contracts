...
describe('TransferExecutor', async function () {
    ...
    if (
      ...
      await verifyBalanceChange(
        ...
        async () =>
          ...
          await transferExecutorContract.transferTest(order.Asset(ETH, '0x', 500), ZERO, accounts1, {
            value: 500,
            gasPrice: 0,
          })
        //)
  ...
  it('should support ERC721 transfers', async () => {
    ...
    acc1AsSigner = transferExecutorContract.connect(wallet1);
    await expectRevert(
      acc1AsSigner.transferTest(order.Asset(ERC721, enc(ghostMarketERC721Token.address, 1), 2), accounts1, accounts2),
      'erc721 value error'
    );
    await acc1AsSigner.transferTest(
      order.Asset(ERC721, enc(ghostMarketERC721Token.address, 1), 1),
      accounts1,
      accounts2
    );

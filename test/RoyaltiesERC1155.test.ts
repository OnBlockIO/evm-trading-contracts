...
describe('ERC1155WithRoyalties Test', () => {
  ...
  beforeEach(async () => {
    ...
    ghostMarketERC1155 = <GhostMarketERC1155>await upgrades.deployProxy(
      GhostMarketERC1155,
  ...
  it('should be able to set address(0) as royalties recipient', async function () {
    ...
    const tx = ghostMarketERC1155.mintGhost(deployer.address, 1, '0x', [{recipient: ZERO, value: 250}], 'ext_uri', '');
  ...
  it('should have no royalties if not set', async function () {
    ...
    await ghostMarketERC1155.mintGhost(deployer.address, 1, '0x', [], 'ext_uri', '');
    const info2 = await ghostMarketERC1155.getRoyalties(1);

...
describe('RoyaltiesRegistry, royalties types test', () => {
  ...
  describe('royalties types are set correctly', () => {
    ...
    it('test royalties type = 2, royalties v2', async () => {
      ...
      tx1 = await tx1.wait();
      expect(await royaltiesRegistry.getRoyaltiesType(ERC721_V2OwnUpgrd.address)).to.be.equal(
        2,
  ...
  describe('royalties types set correctly from external methods', () => {
    ...
    it('forceSetRoyaltiesType + clearRoyaltiesType', async () => {
      ...
      await expect(royaltiesRegistry.connect(signers[accounts[3]]).forceSetRoyaltiesType(token, 1)).to.be.reverted;
      ...
      await expect(royaltiesRegistry.connect(signers[accounts[3]]).clearRoyaltiesType(token)).to.be.reverted;

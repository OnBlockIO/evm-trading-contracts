...
describe('RoyaltiesRegistry, test methods', () => {
  ...
  describe('RoyaltiesRegistry token supports IERC2981:', () => {
    it('Get royalties by token, use RoyaltiesRegistryTest (event)', async () => {
      ...
      expectEvent.inReceipt(part, 'getRoyaltiesTest', Array([[getRoyalties, BigNumber.from(1000)]]));
    });
  ...
  describe('RoyaltiesRegistry methods works:', () => {
    it('simple V2 royalties', async () => {
      ...
      await ERC721_V2OwnUpgrd.mint(accounts[2], erc721TokenId1, [
        [accounts[5], 700],
        [accounts[6], 800],
        [accounts[7], 900],
        [accounts[8], 1000],
      ...
      expectEvent.inReceipt(
        ...
        'getRoyaltiesTest',
        Array([
          [accounts[5], BigNumber.from(700)],
          [accounts[6], BigNumber.from(800)],
          [accounts[7], BigNumber.from(900)],
          [accounts[8], BigNumber.from(1000)],
        ])
      );
    });
    ...
    it('SetRoyaltiesByToken, initialize by Owner, emit get', async () => {
      await royaltiesRegistry.setRoyaltiesByToken(accounts[5], [
        [accounts[3], 600],
        [accounts[4], 1100],
      ...
      expectEvent.inReceipt(
        ...
        'getRoyaltiesTest',
        Array([
          [accounts[3], BigNumber.from(600)],
          [accounts[4], BigNumber.from(1100)],
        ])
      );
    });
  });
  ...
  describe('ExternalProviders test:', () => {
    ...
    it('using royaltiesProvider artBlocks', async () => {
      ...
      expectEvent.inReceipt(txSetAddr, 'OwnershipTransferred', {
        previousOwner: artBlocksAddr,
        newOwner: newArtBlocksAddr,
      });

      await expect(provider.connect(signers[artBlocksAddr]).transferOwnership(newArtBlocksAddr)).to.be.reverted;
      ...
      expectEvent.inReceipt(txChangePercentage, 'ArtblocksPercentageChanged', {
      ...
      //only owner can set %
      await expect(provider.connect(signers[artBlocksAddr]).setArtblocksPercentage(0)).to.be.reverted;

      // _artblocksPercentage can't be over 10000
      await expect(provider.connect(signers[newArtBlocksAddr]).setArtblocksPercentage(100000)).to.be.reverted;
    });

...
describe('ERC1155WithRoyalties', () => {
  ...
  describe('Royalties', async () => {
    ...
    it('can set address(0) as royalties recipient', async function () {
      await erc1155WithRoyalties.mint(deployer.address, 0, 10, ADDRESS_ZERO, 1000);
    ...
    it('each token has the right royalties when minting batch', async function () {
      ...
      const royaltyRecipients = [randomAccount.address, deployer.address, ADDRESS_ZERO, deployer.address];

...
describe('Exchange', async function () {
  ...
  beforeEach(async function () {
    ...
    if (hre.network.name == 'testnet_nodeploy' && do_not_deploy) {
      ...
      testing = await upgrades.deployProxy(
        ExchangeV2,
        [transferProxy.address, erc20TransferProxy.address, 300, protocol, royaltiesRegistryProxy.address],
  ...
  it('cancel erc20 order', async () => {
    ...
    const tx2 = testing.matchOrders(
      left,
      await getSignature(left, accounts1),
      right,
      await getSignature(right, accounts2)
    );
  ...
  it('cancel erc1155 order', async () => {
    ...
    const tx2 = testingAsSigner2.matchOrders(
      left,
      await getSignature(left, accounts1),
      right,
      await getSignature(right, accounts2),
      {from: accounts2, value: 300}
    );
    await expect(tx2).to.be.revertedWith('revert');
  });
  ...
  it('eth orders work, rest is returned to taker (other side)', async () => {
    ...
    if (
      ...
      hre.network.name == 'testnet_nodeploy' ||
  ...
  it('From ETH(DataV1) to ERC721(RoyalytiV1, DataV1) Protocol, NO Origin fees, Royalties', async () => {
    ...
    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      ...
      await verifyBalanceChange(accounts0, 206, async () =>
        //200+6buyerFee (72back)
        verifyBalanceChange(accounts1, -186, async () =>
          //200 - (6+8royalties)
  ...
  it('From ETH(DataV1) to erc721 2981 royalties stndard, NO Origin fees, Royalties', async () => {
    ...
    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      ...
      await verifyBalanceChange(accounts0, 206, async () =>
        //200+6buyerFee (72back)
        verifyBalanceChange(accounts1, -194, async () =>
          //200 - (6+8royalties)
          verifyBalanceChange(accounts2, -6, async () =>
            verifyBalanceChange(protocol, -6, () =>
              matchOrdersSigner.matchOrders(left, '0x', right, signatureRight, {
  ...
  it('From ETH(DataV1) to ERC721(RoyalytiV1, DataV1) Protocol, Origin left and right fees, Royalties', async () => {
    ...
    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      ...
      await verifyBalanceChange(accounts0, 228, async () =>
  ...
  async function testOriginRoyalties(addrOriginLeft, addrOriginRight, originDataLeft_buyer, originDataRight_seller) {
    ...
    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      ...
      if (
        ...
        await verifyBalanceChange(accounts2, 228, async () =>
  ...
  it('should buy ERC1155 with ETH; protocol fee and royalties', async () => {
    ...
    console.log(
      accounts1,
      'accounts1 erc1155 balance before: ',
      (await ghostERC1155.balanceOf(accounts1, erc1155TokenId1)).toString()
    );
    console.log(
      accounts2,
      'accounts2 erc1155 balance before: ',
      (await ghostERC1155.balanceOf(accounts2, erc1155TokenId1)).toString()
    );
    ...
    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      ...
      await verifyBalanceChange(accounts2, 206, async () =>
        ...
        verifyBalanceChange(accounts1, -170, async () =>
          //200 seller - 14
    ...
    console.log(
      accounts1,
      ' accounts1 erc1155 balance after: ',
      (await ghostERC1155.balanceOf(accounts1, erc1155TokenId1)).toString()
    );
    console.log(
      accounts2,
      ' accounts2 erc1155 balance after: ',
      (await ghostERC1155.balanceOf(accounts2, erc1155TokenId1)).toString()
    );
  });

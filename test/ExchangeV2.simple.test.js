...
contract("ExchangeSimpleV2", accounts => {
	...
	describe("matchOrders", () => {
		it("eth orders work, rest is returned to taker", async () => {
			...
			console.log("getSignature: ", await getSignature(left, accounts[1]), right, "0x", { from: accounts[2], value: 201, gasPrice: 0 })
 			*/			
		...
		it("simplest possible exchange works", async () => {
			...
			const chainId= await testing.getChainId()
			console.log("chainId: ",chainId)
			const result = await testing.matchOrders(left, await getSignature(left, accounts[1]), right, await getSignature(right, accounts[2]))
			//console.log("result: ",result)
			//expectEvent(result, 'Debug', {  chainid: chainId, hashSig: accounts[1],  ordermaker:accounts[1]})

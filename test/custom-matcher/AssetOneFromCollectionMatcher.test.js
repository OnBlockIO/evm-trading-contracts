  ...
  describe('Check match by customMatcher Match one from Collection', () => {
    it('Collection COLLECTION <-> ERC1155  matches!', async () => {
      ...
      const result = await assetMatcherCollection
        .connect(operator)
        .matchAssets(order.AssetType(COLLECTION, encoded), order.AssetType(ERC1155, encodedNFT));
    ...
    it('Collection COLLECTION <-> ERC721  matches!', async () => {
      ...
      const result = await assetMatcherCollection
        .connect(operator)
        .matchAssets(order.AssetType(COLLECTION, encoded), order.AssetType(ERC721, encodedNFT));
    ...
    it('Collection COLLECTION <-> ERC20  don`t match', async () => {
      ...
      const result = await assetMatcherCollection
        .connect(operator)
        .matchAssets(order.AssetType(COLLECTION, encoded), order.AssetType(ERC20, encodedERC20));
      expect(BigNumber.from(result[0])).to.be.equal(0);
    });
    ...
    it('Collection COLLECTION <-> COLLECTION  don`t match', async () => {
      ...
      const result = await assetMatcherCollection
        .connect(operator)
        .matchAssets(order.AssetType(COLLECTION, encoded), order.AssetType(COLLECTION, encodedCollection));
      expect(BigNumber.from(result[0])).to.be.equal(0);
    });
    ...
    it('Collection COLLECTION <-> ETH  don`t match', async () => {
      ...
      const result = await assetMatcherCollection
        .connect(operator)
        .matchAssets(order.AssetType(COLLECTION, encoded), order.AssetType(ETH, encodedETH));
      expect(BigNumber.from(result[0])).to.be.equal(0);
    });
  });
});

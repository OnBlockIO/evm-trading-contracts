...
contract("AssetMatcher", accounts => {
	...
	it("setAssetMatcher works", async () => {
		const encoded = enc(accounts[1]);
		// setAssetMatcher is not called and assets can not be matched
		await expectThrow(
			testing.matchAssetsTest(order.AssetType(ERC20, encoded), order.AssetType(id("BLA"), encoded))
		);
		// TestAssetMatcher implements ./contracts/interfaces/IAssetMatcher.sol and a testMatcher function
		const testMatcher = await TestAssetMatcher.new();
		// introduces a new assets that is not predefined in test/assets.js
